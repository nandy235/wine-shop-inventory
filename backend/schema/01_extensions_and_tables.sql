...
DROP TABLE IF EXISTS invoice_brands CASCADE;
...
CREATE TABLE invoice_brands (
    ...
    total_quantity INTEGER GENERATED ALWAYS AS (cases * pack_quantity + bottles) STORED,
    unit_price DECIMAL(10,2),
    total_price DECIMAL(10,2) GENERATED ALWAYS AS (
        COALESCE(unit_price, 0) * (cases * pack_quantity + bottles)
    ) STORED,
    master_brand_id BIGINT REFERENCES master_brands(id), -- For matched brands
    match_confidence DECIMAL(5,2) DEFAULT NULL, -- Matching confidence score (0-100)
    match_method VARCHAR(20) DEFAULT NULL, -- 'exact', 'fuzzy', 'manual'
    matched_at TIMESTAMPTZ DEFAULT NULL,
    CONSTRAINT chk_invoice_nonnegative CHECK (
        cases >= 0 AND bottles >= 0 AND pack_quantity > 0 AND
...
COMMENT ON TABLE invoice_brands IS 'Individual brand items within invoices with auto-linking';
...
COMMENT ON COLUMN invoice_brands.total_quantity IS 'Auto-calculated: cases * pack_quantity + bottles';
COMMENT ON COLUMN invoice_brands.total_price IS 'Auto-calculated: unit_price * total_quantity';
COMMENT ON COLUMN invoice_brands.master_brand_id IS 'Auto-linked via trigger or manually set';
